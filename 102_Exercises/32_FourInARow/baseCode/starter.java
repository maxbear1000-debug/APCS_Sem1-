 	...
 	public static int checkVertical(String[][] board){
		...
		for(int j = 0; j < board.length-3; j++ ){
			for(int i = 0; i <board[0]length ; i++){
				if(board[i][j].equals("A") &&board[i+1][j].equals("A") &&board[i+2][j].equals("A") &&board[i+3][j].equals("A")){
					return 1; 
				}
				else if(board[i][j].equals("B") &&board[i+1][j].equals("B") &&board[i+2][j].equals("B") &&board[i+3][j].equals("B")){
					return 2; 
				}
			}
		}	
