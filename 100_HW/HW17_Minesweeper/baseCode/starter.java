...
class starter{
        ...
        for(int row = 0; row < bombProxGrid.length; row++){
            for(int col = 0; col < bombProxGrid[0].length; col++){
                if(bombProxGrid[row][col] == 9){
                    addAdj(row-1, col-1, bombProxGrid);
                    addAdj(row-1, col, bombProxGrid);
                    addAdj(row-1, col+1, bombProxGrid);
                    addAdj(row, col-1, bombProxGrid);
                    addAdj(row, col+1, bombProxGrid);
                    addAdj(row+1, col-1, bombProxGrid);
                    addAdj(row+1, col, bombProxGrid);
                    addAdj(row+1, col+1, bombProxGrid);
    ...
    */
    public static void reveal(int r, int c, int [][] guessGrid, int[][] bombProxGrid){
        ...
        for(int checkRow = r; checkRow <= r; checkRow++){
            for(int checkCol = c; checkCol <= c; checkCol++){
                if(checkRow >= 0 && checkRow < guessGrid.length &&
                   checkCol >= 0 && checkCol < guessGrid[0].length){
                    guessGrid[checkRow][checkCol] = bombProxGrid[checkRow][checkCol];
                }
            }
        }
        printGrid(guessGrid);
    ...
    */
    public static boolean checkWin(int [][] guessGrid, int [][] bombProxGrid){
        for(int row = 0; row < guessGrid.length; row++){
            for(int col = 0; col < guessGrid[0].length; col++){
                if(bombProxGrid[row][col] != 9 &&
